...
Zero-dependency Node.js/TypeScript library designed to perform deep inspection, verification, compression scoring, Bloom filter validation, and health analysis of Parquet files.
...
- **Bloom Filter Inspector & False Positive Scorer**: Validation of `BLOCK_SPLIT_BLOOM_FILTER` structures and statistical false-positive probability estimation.
- **Page Index & Data Skipping Evaluator**: Analysis of `ColumnIndex` and `OffsetIndex` headers to assess boundary ordering and query skipping capabilities.
- **Dictionary Encoding Efficiency Scorer**: Cardinality ratio and byte-savings evaluation for dictionary-encoded columns.
...
console.log(`Bloom Filters: ${report.bloomFilters?.length ?? 0}`);
console.log(`Page Indexes: ${report.pageIndexes?.length ?? 0}`);
```
